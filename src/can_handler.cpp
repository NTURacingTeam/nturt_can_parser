...
void CanHandler::init(std::string _file) {
...
void CanHandler::publish(const FramePtr &_frame, const PublishFun publish_fun) const {
...
bool CanHandler::publish(const std::string &_name, const PublishFun publish_fun) const {
    try {
...
void CanHandler::periodic_publish(const double &_dt, const PublishFun publish_fun) const {
...
DataPtr CanHandler::update_data(const std::string &_name, const double &_value) {
    try {
...
FramePtr CanHandler::update_frame(const int &_id, const boost::array<uint8_t, 8> &_data) {
    try {
...
DataPtr CanHandler::get_data(const std::string &_name) const {
    try {
...
FramePtr CanHandler::get_frame(const int &_id) const {
    try {
...
FramePtr CanHandler::get_frame(const std::string &_name) const {
    try {
...
Dataset CanHandler::get_dataset() const {
    return dataset_;
}

IdFrameset CanHandler::get_id_frameset() const {
    return id_frameset_;
}

NameFrameset CanHandler::get_name_frameset() const {
...
void decode(DataPtr &_data, const boost::array<uint8_t, 8> &_raw_data) {
    ...
    if(_data->is_signed_) {
        ...
        if(is_negative) {
            ...
            // bit to uint64_t's most significant bit) for casting uint64_t to int64_t
...
void encode(const DataPtr &_data, boost::array<uint8_t, 8> &_raw_data) {
    ...
    // WARNNING: doew NOT check for overflow
    uint64_t compose = (_data->last_data_ - _data->offset_) / _data->resolution_;
    ...
    if(_data->is_byte_) {
        ...
        // calculated by removing the last "end_bit" bit by modding 2 ^ end_bit, then remove first "start_bit" bit by shifting to right start_bit
        uint8_t clear_mask = (_raw_data[_data->start_byte_] & (pow2[_data->end_bit_] - 1)) >> _data->start_bit_;
