...
SocketCanReceiver::SocketCanReceiver(const rclcpp::NodeOptions &_options)
    : Node("socket_can_receiver_node", _options),
      can_pub_(
          this->create_publisher<can_msgs::msg::Frame>("/from_can_bus", 100)),
  ...
  try {
    receiver_ =
        std::make_unique<drivers::socketcan::SocketCanReceiver>(interface_);
  ...
  receiver_thread_ =
      std::make_unique<std::thread>(&SocketCanReceiver::receive, this);
}

SocketCanReceiver::~SocketCanReceiver() { receiver_thread_->join(); }
